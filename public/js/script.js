...
function updateCarousel() {
    // Oculta todas las diapositivas y elimina la clase activa
    slides.forEach((slide, index) => {
        slide.classList.remove('active');
        indicators[index].classList.remove('active');
    });

    // Muestra la diapositiva actual y agrega la clase activa al indicador correspondiente
...
function nextSlide() {
    ...
    currentSlide = (currentSlide + 1) % slides.length; // Cicla al inicio
    updateCarousel();
}

function prevSlide() {
    // Retrocede a la diapositiva anterior
    currentSlide = (currentSlide - 1 + slides.length) % slides.length; // Cicla al final
    updateCarousel();
}
